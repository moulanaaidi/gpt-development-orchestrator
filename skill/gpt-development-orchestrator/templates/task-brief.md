# Worker Task: <ID> - <short title>

## Goal
<One bounded implementation outcome>

## Context
<Relevant repository facts, instructions, interfaces, and decisions>

## Owned write set
- `path/to/owned/file`

Do not modify paths outside this list. Do not commit, push, merge, deploy, change credentials, or perform production operations.

## Constraints and assumptions
- <Required behavior, compatibility, security, or design constraints>
- <Assumptions; stop and report if one proves false>

## Acceptance
- [ ] <Observable result>
- [ ] <Focused test/check and command>

## Return to Sol
Report status; changed paths; exact commands run and results; acceptance evidence; deviations; unresolved risks or test gaps; and any required follow-up. Do not claim checks that were not run.
